...
import com.lzpeng.minimal.common.core.response.CommonCode;
import com.lzpeng.minimal.common.core.response.Result;
import com.lzpeng.minimal.common.core.response.ResultCode;
...
public class OAuth2ExceptionTranslator extends DefaultWebResponseExceptionTranslator {
     ...
     * @param oAuth2Exception OAuth2异常
     * @return 转换为通用格式
     */
     ...
     * @param oAuth2Authentication OAuth2 异常
     * @return 转换成自己的格式
     */
    private Map<String, Object> getResultData(OAuth2Exception oAuth2Authentication) {
        ...
        if ("UserDetailsService returned null, which is an interface contract violation".equals(errorMessage)) {
            ...
            if (errorMessage.startsWith("Access token expired:")){
                map.put("msg", "TOKEN过期");
                map.put("needReLogin", true);
            } else {
